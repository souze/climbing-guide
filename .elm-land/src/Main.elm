...
initPageAndLayout model =
    case Route.Path.fromUrl model.url of
        ...
        Route.Path.Crags ->
            let
                page : Page.Page Pages.Crags.Model Pages.Crags.Msg
                page =
                    Pages.Crags.page model.shared (Route.fromUrl () model.url)
            ...
            { page = 
                Tuple.mapBoth
                    Main.Pages.Model.Crags
                    (Effect.map Main.Pages.Msg.Crags >> fromPageEffect model)
                    ( pageModel, pageEffect )
            , layout = Nothing
            }

        Route.Path.Crags_Id_ params ->
            let
                page : Page.Page Pages.Crags.Id_.Model Pages.Crags.Id_.Msg
                page =
                    Pages.Crags.Id_.page model.shared (Route.fromUrl params model.url)
            ...
            { page = 
                Tuple.mapBoth
                    (Main.Pages.Model.Crags_Id_ params)
                    (Effect.map Main.Pages.Msg.Crags_Id_ >> fromPageEffect model)
                    ( pageModel, pageEffect )
            , layout = Nothing
            }
...
updateFromPage msg model =
    case ( msg, model.page ) of
        ...
        ( Main.Pages.Msg.Crags pageMsg, Main.Pages.Model.Crags pageModel ) ->
            Tuple.mapBoth
                Main.Pages.Model.Crags
                (Effect.map Main.Pages.Msg.Crags >> fromPageEffect model)
                (Page.update (Pages.Crags.page model.shared (Route.fromUrl () model.url)) pageMsg pageModel)

        ( Main.Pages.Msg.Crags_Id_ pageMsg, Main.Pages.Model.Crags_Id_ params pageModel ) ->
            Tuple.mapBoth
                (Main.Pages.Model.Crags_Id_ params)
                (Effect.map Main.Pages.Msg.Crags_Id_ >> fromPageEffect model)
                (Page.update (Pages.Crags.Id_.page model.shared (Route.fromUrl params model.url)) pageMsg pageModel)
...
toLayoutFromPage model =
    case model.page of
        ...
        Main.Pages.Model.Crags pageModel ->
            Route.fromUrl () model.url
                |> Pages.Crags.page model.shared
                |> Page.layout pageModel
                |> Maybe.map (Layouts.map (Main.Pages.Msg.Crags >> Page))

        Main.Pages.Model.Crags_Id_ params pageModel ->
            Route.fromUrl params model.url
                |> Pages.Crags.Id_.page model.shared
                |> Page.layout pageModel
                |> Maybe.map (Layouts.map (Main.Pages.Msg.Crags_Id_ >> Page))
    ...
    let
        ...
        subscriptionsFromPage =
            case model.page of
                ...
                Main.Pages.Model.Crags pageModel ->
                    Page.subscriptions (Pages.Crags.page model.shared (Route.fromUrl () model.url)) pageModel
                        |> Sub.map Main.Pages.Msg.Crags
                        |> Sub.map Page

                Main.Pages.Model.Crags_Id_ params pageModel ->
                    Page.subscriptions (Pages.Crags.Id_.page model.shared (Route.fromUrl params model.url)) pageModel
                        |> Sub.map Main.Pages.Msg.Crags_Id_
                        |> Sub.map Page
...
viewPage model =
    case model.page of
        ...
        Main.Pages.Model.Crags pageModel ->
            Page.view (Pages.Crags.page model.shared (Route.fromUrl () model.url)) pageModel
                |> View.map Main.Pages.Msg.Crags
                |> View.map Page

        Main.Pages.Model.Crags_Id_ params pageModel ->
            Page.view (Pages.Crags.Id_.page model.shared (Route.fromUrl params model.url)) pageModel
                |> View.map Main.Pages.Msg.Crags_Id_
                |> View.map Page
...
toPageUrlHookCmd model routes =
    ...
    case model.page of
        ...
        Main.Pages.Model.Crags pageModel ->
            Page.toUrlMessages routes (Pages.Crags.page model.shared (Route.fromUrl () model.url)) 
                |> List.map Main.Pages.Msg.Crags
                |> List.map Page
                |> toCommands

        Main.Pages.Model.Crags_Id_ params pageModel ->
            Page.toUrlMessages routes (Pages.Crags.Id_.page model.shared (Route.fromUrl params model.url)) 
                |> List.map Main.Pages.Msg.Crags_Id_
                |> List.map Page
                |> toCommands
